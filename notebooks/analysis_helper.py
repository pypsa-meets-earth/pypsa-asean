...
def strip_network(n, scenario=""):
    ...
    if scenario == "no-AIMS":
        links_project = m.links[
            (m.links.build_year >= 2025)
            & ~(
                m.links.bus0.map(m.buses.country).isin(["ID"])
                & m.links.bus1.map(m.buses.country).isin(["ID"])
            )
        ].index
        lines_project = m.lines[
            (m.lines.build_year >= 2025)
            & ~(
                m.lines.bus0.map(m.buses.country).isin(["ID"])
                & m.lines.bus1.map(m.buses.country).isin(["ID"])
            )
        ].index
