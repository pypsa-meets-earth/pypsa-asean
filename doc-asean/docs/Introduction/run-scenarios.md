...
## (Alternative) Run the Tutorial Scenario

Alternatively, you can execute a tutorial model designed for quick runtimes using open-source solvers. The file `configs/tutorial.asean.yaml` provides a suitable setup.
```yaml
# configs/tutorial.asean.yaml
...
```bash
snakemake solve_sector_networks --configfile configs/config.asean.yaml configs/tutorial.asean.yaml -call
```

* `solve_sector_networks` specifies the rule to execute
* `--configfile` indicates the configuration file that overrides the default PyPSA-Earth settings
